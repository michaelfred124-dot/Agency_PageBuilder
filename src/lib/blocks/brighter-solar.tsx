             ...
             return (
               <div key={i} className="bg-[#13284a] rounded-2xl overflow-hidden group flex flex-col border border-white/5">
                 <div className="h-48 relative overflow-hidden bg-black/40">
                   <Image src={(service.img) || "https://images.unsplash.com/photo-1509391365360-2e959784a276?w=800&q=80"} alt={service.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" referrerPolicy="no-referrer" fill />
          ...
          {(props.reviews || []).map((review: any, i: number) => (
            <div key={i} className="bg-transparent border border-white/20 rounded-2xl p-8 flex flex-col items-center text-center relative group hover:bg-white/5 transition-colors">
              ...
              <div className="mt-auto flex items-center gap-4 relative z-10">
                <Image src={(review.img) || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=800&q=80"} alt={review.name} className="w-12 h-12 rounded-full object-cover border-2 border-white/20" referrerPolicy="no-referrer" width={48} height={48} />
...
export { BS_SCHEMAS } from './brighter-solar.schemas';
