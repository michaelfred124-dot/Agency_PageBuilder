...
const inputClass =
  'w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 focus:outline-none transition font-medium';
...
function WelcomeInner() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pt-24 lg:pt-28 pb-24 px-4 lg:px-6 relative overflow-hidden font-sans">
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[450px] bg-gradient-to-r from-indigo-200/30 via-sky-200/30 to-emerald-200/30 blur-[150px] rounded-full pointer-events-none z-0" />

      <div className="max-w-2xl mx-auto relative z-10">
        {/* Header */}
        <div className="mb-10 text-center space-y-4">
          <div className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.25em] font-extrabold text-indigo-700 bg-indigo-50 border border-indigo-200 px-4 py-1.5 rounded-full shadow-sm">
          ...
          <p className="text-slate-600 text-sm md:text-base font-normal max-w-lg mx-auto">
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 md:p-10 shadow-xl shadow-slate-200/50 relative">
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                      Business Name <span className="text-indigo-600">*</span>
                    </label>
                    <input
                        ...
                        <button
                          ...
                          className={`p-3 rounded-2xl border text-left transition-all flex items-center gap-3 ${
                            industryKey === tmpl.key
                              ? 'border-indigo-600 bg-indigo-50/50 shadow-sm text-indigo-950 font-bold'
                ...
                <button
                  type="submit"
                  className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs uppercase tracking-wider py-4 rounded-2xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer hover:shadow-indigo-200 hover:-translate-y-0.5 active:translate-y-0"
                >
              ...
              >
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="w-1/3 border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold text-xs uppercase tracking-wider py-4 rounded-2xl transition-all"
                  >
                    Back
                  </button>
                  <button
                    type="submit" disabled={submitting}
                    className="w-2/3 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white font-black text-xs uppercase tracking-wider py-4 rounded-2xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer hover:-translate-y-0.5 active:translate-y-0"
                  >
                    {submitting
                      ? <><Loader2 className="w-4 h-4 animate-spin" /> Setting up…</>
                      : <>Start My Build <ArrowRight className="w-4 h-4" /></>}
                  </button>
                </div>
            ...
            {step === 4 && (
              <motion.div
                ...
                className="text-center space-y-5 py-8"
              >
                <div className="w-16 h-16 bg-indigo-100 rounded-2xl flex items-center justify-center mx-auto text-3xl">
                  📧
                </div>
                <div className="space-y-2">
                ...
                <Link
                  href="/login"
                  className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs uppercase tracking-wider px-6 py-3 rounded-2xl transition-all"
                >
