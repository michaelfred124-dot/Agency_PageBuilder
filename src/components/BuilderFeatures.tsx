...
export default function BuilderFeatures() {
  ...
  return (
    <section id="features" className="py-20 lg:py-32 bg-white text-[#1a1a1a] px-6 relative overflow-hidden border-b border-black/5">
      {/* Decorative dot background */}
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, #1a1a1a 1px, transparent 0)', backgroundSize: '24px 24px' }} />
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        ...
        <div className="flex flex-col space-y-4 mb-16 lg:mb-24 text-center items-center">
          ...
          <p className="max-w-2xl text-[#1a1a1a]/60 text-base md:text-lg font-normal pt-2">
        ...
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-14 items-start">
          ...
          <div className="lg:col-span-7 flex flex-col space-y-6">
            ...
            {/* Control Bar */}
            <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-black/[0.02] border border-black/8 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.05)]">
              <div className="flex items-center gap-3">
                <span className="w-2.5 h-2.5 rounded-full bg-black/10" />
                <span className="w-2.5 h-2.5 rounded-full bg-black/10" />
                <span className="w-2.5 h-2.5 rounded-full bg-black/10" />
                <span className="text-[10px] font-bold text-[#1a1a1a]/40 tracking-wider uppercase ml-2 hidden sm:inline">LIVE BUILDER SIMULATOR</span>
              </div>
              
              {/* Viewport Selectors */}
              <div className="flex items-center gap-1 bg-black/5 rounded-lg p-1 border border-black/8">
                <button 
                  onClick={() => setActiveDevice('desktop')}
                  className={`p-1.5 rounded transition-all cursor-pointer ${activeDevice === 'desktop' ? 'bg-white text-[#1a1a1a] shadow-sm' : 'text-[#1a1a1a]/40 hover:text-[#1a1a1a]'}`}
                  title="Desktop View"
                >
                ...
                <button 
                  ...
                  className={`p-1.5 rounded transition-all cursor-pointer ${activeDevice === 'tablet' ? 'bg-white text-[#1a1a1a] shadow-sm' : 'text-[#1a1a1a]/40 hover:text-[#1a1a1a]'}`}
                  title="Tablet View"
                >
                ...
                <button 
                  ...
                  className={`p-1.5 rounded transition-all cursor-pointer ${activeDevice === 'mobile' ? 'bg-white text-[#1a1a1a] shadow-sm' : 'text-[#1a1a1a]/40 hover:text-[#1a1a1a]'}`}
                  title="Mobile View"
                >
            ...
            <div className="relative w-full flex justify-center items-start">
              ...
              <div className="absolute -left-4 top-20 z-20 w-38 hidden xl:flex flex-col gap-2 p-3 bg-white/95 backdrop-blur-md border border-black/10 rounded-xl shadow-xl select-none">
                <div className="text-[9px] font-bold uppercase text-[#1a1a1a]/40 tracking-wider">Blocks</div>
                <div className="flex items-center gap-2 p-1.5 bg-black/5 border border-black/8 rounded text-[10px] font-medium text-[#1a1a1a]/80">
                  <GripVertical className="w-3 h-3 text-[#1a1a1a]/20 shrink-0" />
                  Hero Section
                </div>
                <div className="flex items-center gap-2 p-1.5 bg-black/5 border border-black/8 rounded text-[10px] font-medium text-[#1a1a1a]/80">
                  <GripVertical className="w-3 h-3 text-[#1a1a1a]/20 shrink-0" />
                ...
                <div className="flex items-center gap-2 p-1.5 bg-black/5 border border-black/5 rounded text-[10px] font-medium text-[#1a1a1a]/30">
                  <GripVertical className="w-3 h-3 text-[#1a1a1a]/10 shrink-0" />
                  Calendly Block
                </div>
              </div>

              {/* Floating Styling Inspector Panel (Simulation) */}
              <div className="absolute -right-4 top-32 z-20 w-44 hidden xl:flex flex-col gap-3 p-3 bg-white/95 backdrop-blur-md border border-black/10 rounded-xl shadow-xl select-none">
                <div className="text-[9px] font-bold uppercase text-[#1a1a1a]/40 tracking-wider">Canvas Theme</div>
                <div className="flex gap-1.5">
                  <button 
                    onClick={() => setSimulatedTheme('light')}
                    className={`w-6 h-6 rounded-full bg-stone-100 border-2 transition-all cursor-pointer ${simulatedTheme === 'light' ? 'border-[#FF7F11] scale-110' : 'border-black/10'}`}
                    title="Light theme"
                  />
                  <button 
                    onClick={() => setSimulatedTheme('sand')}
                    className={`w-6 h-6 rounded-full bg-[#F4F1EA] border-2 transition-all cursor-pointer ${simulatedTheme === 'sand' ? 'border-[#FF7F11] scale-110' : 'border-black/10'}`}
                    title="Sand theme"
                  />
                  <button 
                    onClick={() => setSimulatedTheme('dark')}
                    className={`w-6 h-6 rounded-full bg-zinc-900 border-2 transition-all cursor-pointer ${simulatedTheme === 'dark' ? 'border-[#FF7F11] scale-110' : 'border-black/10'}`}
                    title="Dark theme"
                  />
                </div>
                <div className="border-t border-black/10 pt-2">
                  <div className="text-[9px] font-bold uppercase text-[#1a1a1a]/40 tracking-wider mb-1">Padding</div>
                  <div className="bg-black/5 p-1 text-[10px] font-semibold text-center border border-black/8 rounded text-[#1a1a1a]/80">
                    Py-24 (Large)
                  </div>
                </div>
              </div>
              ...
              <motion.div 
                ...
                className="w-full bg-white border border-black/10 rounded-2xl overflow-hidden shadow-2xl relative transition-all"
              >
            ...
            {/* Mobile helpful text */}
            <div className="flex items-center gap-2 bg-black/5 p-3 rounded-xl border border-black/8 text-xs font-semibold text-[#1a1a1a]/50 md:hidden justify-center text-center">
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-[#1a1a1a]/50 leading-relaxed font-normal">
                      {item.desc}
                    </p>
                  </div>
                </motion.div>
        ...
        <div className="mt-20 rounded-[32px] p-8 md:p-12 text-center flex flex-col items-center luxury-card relative overflow-hidden">
          ...
          <h3 className="text-2xl md:text-4xl font-extrabold uppercase tracking-tight mb-4 max-w-2xl leading-none text-[#1a1a1a]">
            Ready to build your local authority?
          </h3>
          <p className="text-sm md:text-base font-normal text-[#1a1a1a]/60 mb-8 max-w-xl">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <a 
              href="/pricing"
              className="px-6 py-3 border border-black/10 text-[#1a1a1a] hover:bg-black/5 rounded-full font-semibold text-xs transition-all cursor-pointer"
            >
              Compare Plans
            </a>
          </div>
