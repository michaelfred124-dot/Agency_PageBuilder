...
export default function FAQ() {
  ...
  return (
    <section className="py-20 lg:py-32 bg-[#FAFAFA] px-4 lg:px-6 relative overflow-hidden">
      <AnimatedSquiggles />
      <div className="max-w-[1000px] mx-auto relative z-10">
        <div className="text-center mb-16 lg:mb-24">
          <span className="text-xs lg:text-sm uppercase tracking-[0.4em] font-bold text-[#D4AF37]">FAQ</span>
          <h2 className="text-4xl md:text-6xl font-extrabold uppercase tracking-tight text-[#1a1a1a] mt-4">
        ...
        <div className="space-y-4 lg:space-y-6">
          {FAQS.map((faq, index) => (
            <motion.div 
              key={index} 
              className="border border-black/8 rounded-2xl overflow-hidden bg-white transition-all duration-300 hover:bg-black/[0.01] shadow-sm"
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full p-6 lg:p-8 flex justify-between items-center text-left cursor-pointer"
              >
                <h3 className="text-lg lg:text-2xl font-extrabold tracking-tight pr-8 text-[#1a1a1a]">
                  {faq.question}
                </h3>
                <div 
                  className="w-8 h-8 flex-shrink-0 rounded-full border border-black/10 flex items-center justify-center transition-all duration-300"
                  style={{ 
                ...
                >
                  {openIndex === index ? <Minus className="w-4 h-4 text-white" strokeWidth={2.5} /> : <Plus className="w-4 h-4 text-[#1a1a1a]" strokeWidth={2.5} />}
                </div>
              </button>
              ...
              <AnimatePresence>
                {openIndex === index && (
                  ...
                  >
                    <div className="p-6 lg:p-8 pt-0 text-sm lg:text-base font-normal text-[#1a1a1a]/60 leading-relaxed border-t border-black/5 mt-4">
                      {faq.answer}
                    </div>
                  </motion.div>
                )}
