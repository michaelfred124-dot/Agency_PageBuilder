      ...
      <nav className="sticky top-0 z-50 bg-white border-b border-black/5">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-16">
          <Link href={BASE} className="flex items-center gap-2">
            ...
            <span className="font-serif italic text-lg tracking-wider" style={{ color: DARK }}>Solstice</span>
            <span className="text-[8px] font-bold uppercase tracking-widest text-black/25 ml-1">Yoga & Wellness</span>
          </Link>
          <div className="hidden lg:flex items-center gap-8">
            {LINKS.map(({ label, href }) => (
              <Link key={href} href={href}
                ...
                style={{ color: pathname === href ? DARK : undefined, borderColor: SAGE }}>
                {label}
              </Link>
            ))}
          </div>
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 top-16 z-50 flex flex-col p-10 gap-7 bg-white">
          ...
          <Link href={`${BASE}/contact`} onClick={() => setOpen(false)}
            ...
            style={{ backgroundColor: SAGE }}>Reserve a Mat</Link>
        </div>
      )}
    </>
  );
